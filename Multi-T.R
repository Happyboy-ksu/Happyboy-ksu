...
si.fun=function(q,I,m,pr,tau,b,omega)
 ...
 ##est_t=TMIX_LMM(z_yub,z_xub, z_uub,pr.init,beta.init,df.init,sigma.init,si.init)
  ...
  for(ii in 1:rp)
   ...
   rn=rbinom(1,I,0.5) # randomly generate index number for component1. 
   ...
   if (datatype=="CN") # Contaminated Normal  
    {
     ...
     sigma=diag(1,r)
     u1=runif(I,0,1)
     er=(u1<0.95)*mvrnorm(I,mu,sigma)+(u1>0.95)*mvrnorm(I,mu,25*sigma)
...
## ----saveoutput------------------------------------------------------------------
 ...
 fzname=paste0("C:\\Users\\weixi\\Dropbox\\LmmSimul\\",zname)
 tname=paste0("T-",datatype,"-n",as.character(r),"-I",as.character(I),".Rdata")
 ftname=paste0("C:\\Users\\weixi\\Dropbox\\LmmSimul\\",tname)
