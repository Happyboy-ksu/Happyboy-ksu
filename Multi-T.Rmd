---
title: "Mixture of LMM Using Multivariate $t$-Distribution"
author: "Weixing Song"
date: "2024-08-24"
output: pdf_document
---
...
This is the one of three R Programs developed for the paper *Robust Mixture of Linear Mixed Modeling via Multivariate Laplace Distribution*. The estimation method is based upon the multivariate t-distribution, orginally written by Bai, Chen and Yao.\vskip 0.1in
...
# Generating 3-way array of unbalanced response $Y_{r\times 1\times I}$.
...
## Functions Used in EM-T Algorightm
...
$\tau$ function: estimate tau's $I\times m$, since we don't know which comp $i$ comes from.
...
# $\Omega$-Function
 ...
 $\Omega$ is a 4 way array with dim: $q\times q \times m \times I$.
...
```
\newpage
<!-- To use $\beta$ function, use the following syntax -->
<!-- ```{r beta_usage} -->
<!--   -->
<!-- ``` -->
...
## $\sigma$ Function
 ...
 Fix the upadated $beta$ value, update sigma
...
## $\Psi$-Function
 ...
 The $\Psi$ is a $q\times q\times m$ 3-way array.
...
# Whole function of  EM_t algorithm
...
```{r simulation}
  ...
  I=100   # total subject
  datatype="N" # N, T1,T3,T5,L,CN
    
  rp=5; # number of replications
  ...
  ####################################################### 
   ...
   if(datatype=="L")
    {dfu=0*I81+0*I82+0*I42+0*I44} else    
   if(datatype=="CN")
