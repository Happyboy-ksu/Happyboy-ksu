...
ev <- new.env()
load("C:\\Users\\weixi\\Dropbox\\LmmSimul\\RealData\\bigy.RData",ev)

bigy = get("bigy",envir=ev)
...
for(jjj in 1:rpnum){
  ...
  # Load Data: No Outlier

  z_xubb=z_uubb=z_xub[,,indexb[,jjj]]
  yboot=array(bigy[,,,jjj],c(12,1,252))
  ...
  paresm0[,,jjj]=rbind(est_L$pr.theta,est_L$beta.theta)
 }


 fLname=paste0("C:\\Users\\weixing\\Dropbox\\LmmSimul\\","L-RealData.Rdata")
 save(paresm0,file=fLname)
