...
est_Ldfsq1=est_Ldfsq2=est_Ldfsq3=array(0,c(pm,m,rp))
...
for (ii in 1:rp)
{
  ...
  est_Ldfsq1[,,ii]=(est_Llbpar[,,ii]-est_init_tr1[,,ii])^2
  est_Ldfsq2[,,ii]=(est_Llbpar[,,ii]-est_init_tr2[,,ii])^2
  est_Ldfsq3[,,ii]=(est_Llbpar[,,ii]-est_init_tr3[,,ii])^2
  
  compsum=apply(cbind(est_Ldfsq1[,,ii],est_Ldfsq2[,,ii],est_Ldfsq3[,,ii]),2,sum)
  ...
  if(aindex==1)
  {
    ...
    est_Llb[,3,ii]=est_Llbpar[,3,ii]
  } else 
    if (aindex==2)
    {
      est_Llb[,1,ii]=est_Llbpar[,1,ii]
      est_Llb[,3,ii]=est_Llbpar[,2,ii]
      est_Llb[,2,ii]=est_Llbpar[,3,ii]
    } else 
      if (aindex==3)
      {
        ...
        if(aindex==4)
        {
          est_Llb[,2,ii]=est_Llbpar[,1,ii]
          est_Llb[,3,ii]=est_Llbpar[,2,ii]
          est_Llb[,1,ii]=est_Llbpar[,3,ii]
        } else
          if (aindex==5)
          {
            est_Llb[,3,ii]=est_Llbpar[,1,ii]
            est_Llb[,1,ii]=est_Llbpar[,2,ii]
            est_Llb[,2,ii]=est_Llbpar[,3,ii]
          } else 
            if(aindex==6)
...
Lname=paste0("real_",as.character(numoutlier),"_L.Rdata")
fLname=paste0("C:\\Users\\weixing\\Dropbox\\LmmSimul\\realdata",zname)
save(est_Llb,file=fLname)

mean_L=apply(est_Llb[1:3,1:3,],c(1,2),mean)  # Multivariate L

sd_L=apply(est_Llb[1:3,1:3,],c(1,2),sd)  # Multivariate L
...
for(j in seq(3))
{
  for(i in seq(3))
  {
    boxplot.matrix(cbind(est_Llb[i,j,]),
                   names=c('L'),xlab=xlabs[i+3*(j-1)])
  }
}
