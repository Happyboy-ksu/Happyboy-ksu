# Simulation Study Using Multivariate t-Distribution from Bai et al. (2016)
# for the paper
...
si.fun=function(q,I,m,pr,tau,b,omega)
 ...
 TMIX_LMM<-function(z_yub,z_xub,z_uub,pr.init,beta.init,df.init,sigma.init,si.init)
  ...
  invisible (list(pr.theta=pr.new,sigma.theta=sigma.new,
             beta.theta=beta.new,df.theta=df.new,si.theta=si.new,
             loglik=loglik.all,dif=dif))
 }#end of function
 ##est_t=TMIX_LMM(z_yub,z_xub, z_uub,pr.init,beta.init,df.init,sigma.init,si.init)

  time0=Sys.time()
r=8 # repeated measures for each subject in balanced case
I=200   # total subject
...
for(datatype in c("N","T3","T5","L","CN"))
  ...
  # Repeated measures for each subject in balanced case
  ...
  # The covariance matrix Psi of bi
  ...
  if(datatype=="L") # 2. Multivariate  Laplace
   {  
     ###################################
   ...
   if((datatype=="T1")|(datatype=="T3")|(datatype=="T5")) # Multivariate t
    {  
     #############################
   ...
   if (datatype=="CN") # Contaminated Normal  
    {
     crate=0.95
     mu=rep(0,r);
     sigma=diag(1,r)
     ...
     sigma=diag(1,q)
     u2=runif(I,0,1)
     b12=(u2<crate)*mvrnorm(I,mu1,sigma)+(u2>crate)*mvrnorm(I,mu2,25*sigma)
    } else 
  ...
  ####################################################### 
   ...
   if(datatype=="N")
     {dfu=12.46*I81+12.46*I82+10.65*I42+12.46*I44} else 
   if(datatype=="T1")
     {dfu=1.605*I81+1.605*I82+2.575*I42+1.488*I44} else 
   if(datatype=="T3")    
     {dfu=5.665*I81+3.832*I82+6.149*I42+5.252*I44} else    
   if(datatype=="T5")
     {dfu=6.716*I81+9.868*I82+7.199*I42+7.766*I44} else    
   if(datatype=="L")
     {dfu=2.154*I81+2.060*I82+2.578*I42+2.586*I44} else    
   if(datatype=="CN")
     {dfu=4.839*I81+4.133*I82+5.665*I42+3.015*I44}     
   ...
   ######################################
   # use  df=1000 to get estimate
   ######################################

   pr.theta=c(0.4,0.6);           #probability of ith obs in jth component.
   beta.theta=beta12;             #fixed parameters
   df.theta=c(1000,1000);           #degree freedoms for m components.
   sigma.theta=matrix(c(1,1),2);  #variance about errors
   si.theta=array(cbind(sigma1,sigma2),c(q,q,2));#variance about bij's
    ...
    for (jj in 1:m)
      { ## start loop for group;
       ...
       est_tpar[,jj,ii]=rbind(est_t$pr.theta[jj],
                              ...
                              t(est_t$sigma.theta)[,jj],
                              as.matrix(as.vector(est_t$si.theta[,,jj]),ncol=1))

       est_zpar[,jj,ii]=rbind(est_z$pr.theta[jj],
                              as.matrix(est_z$beta.theta[,jj],ncol=1),
                              t(est_z$sigma.theta)[,jj],
                              as.matrix(as.vector( est_z$si.theta[,,jj]),ncol=1))
 ...
 ## Check the label and then get the Mse and Bias
  ...
  est_tdfsq=array(0,c(pm,m,rp))
  est_zdfsq=array(0,c(pm,m,rp))
  ...
  for (ii in 1:rp)
   {
    ...
    est_tdfsq[,,ii]=(est_tlbpar[,,ii]-est_init_tr[,,ii])^2

    est_zdfsq[,,ii]=(est_zlbpar[,,ii]-est_init_tr[,,ii])^2


    if(sum(est_tdfsq[,1,ii])>sum(est_tdfsq[,2,ii]))
      {
    ...
    if(sum(est_zdfsq[,1,ii])>sum(est_zdfsq[,2,ii]))
      {
...
## ----medse-----------------------------------------------------------------------
 mdese_t=apply((est_tlb[1:5,1:2,]-est_init[1:5,1:2,])^2,c(1,2),median)  # Multivariate T
 mdese_n=apply((est_zlb[1:5,1:2,]-est_init[1:5,1:2,])^2,c(1,2),median)  # Multivariate N
 medeff=mdese_n/mdese_t


## ----mse-------------------------------------------------------------------------
 mse_t=apply((est_tlb[1:5,1:2,]-est_init[1:5,1:2,])^2,c(1,2),mean)  # Multivariate T
 mse_n=apply((est_zlb[1:5,1:2,]-est_init[1:5,1:2,])^2,c(1,2),mean)  # Multivariate N
 mseeff=mse_n/mse_t
...
## ----boxplot---------------------------------------------------------------------
 ...
 #                    names=c('T','N'),xlab=xlabs[i+5*(j-1)])
 #    }
 #  }
  }